...
This repository now supports two side-by-side flows that use the same two Foundry models (`gpt-5.4-mini` and `phi-4`):
...
`terraform/` remains a separate root from the standalone package and now hosts the two Front Door chat entry points.
...
| [terraform/frontdoor.tf](terraform/frontdoor.tf) | Shared Front Door profile and routes for both chat apps, plus MLflow gateway endpoint |
...
3. Keep `mlflow_model_alias`, `litellm_chat1_model_alias`, and `litellm_chat1_phi_model_alias` aligned to the same two Foundry aliases so chat1/chat2 test identical model paths.