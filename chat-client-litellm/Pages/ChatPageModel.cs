...
public abstract class ChatPageModel(LiteLlmChatService chatService, ILogger logger) : PageModel
{
    ...
    [StringLength(LiteLlmChatService.MaxMessageLength,
        ...
        try
        {
            ...
            Conversation = saved
                .Where(turn => turn is not null && (turn.Role is "user" or "assistant")
                    ...
                    && turn.Content.Length <= LiteLlmChatService.MaxReplyLength)
                .Select(turn => turn!)
                .ToList();
    ...
    private static void TrimConversation(List<ChatTurn> turns)
    {
        ...
        while (turns.Count > LiteLlmChatService.MaxHistoryTurns ||
            characters > LiteLlmChatService.MaxHistoryCharacters)
        {
