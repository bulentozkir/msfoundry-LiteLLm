...
public abstract class ChatPageModel(MlflowChatService chatService, ILogger logger) : PageModel
{
    ...
    [StringLength(MlflowChatService.MaxMessageLength,
        ...
        try
        {
            ...
            Conversation = saved
                .Where(turn => turn is not null && (turn.Role is "user" or "assistant")
                    ...
                    && turn.Content.Length <= MlflowChatService.MaxReplyLength)
                .Select(turn => turn!)
                .ToList();
    ...
    private static void TrimConversation(List<ChatTurn> turns)
    {
        ...
        while (turns.Count > MlflowChatService.MaxHistoryTurns ||
            characters > MlflowChatService.MaxHistoryCharacters)
        {
