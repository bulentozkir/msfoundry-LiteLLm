...
<#
.SYNOPSIS
Operate the standalone root without editing HCL. See litellm.md before use.
.DESCRIPTION
...
function Assert-SafePlan {
    ...
    if ($Action -in 'Stage', 'Promote', 'Rollback') {
        foreach ($change in $changes) {
            ...
            if ($change.address -ne 'module.litellm.azurerm_container_app.this' -or ($change.change.actions -join ',') -ne 'update') {
...
try {
    ...
    switch ($Action) {
        ...
        'Stage' {
            if (!$MigrationReviewed) { throw 'Read the migration gate in litellm.md. Supply -MigrationReviewed only after backup and old/new schema compatibility tests.' }
